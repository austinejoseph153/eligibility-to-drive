...
function checkEligibility(e){
    ...
    if(age >= 70){
        var message  = `age <strong>${age}</strong> is too old to drive`   // message to be displayed to user
        displayResult(result,error,info,message=message,status="info")
    }
    // if age is greater than  18 but less than 70 the the user is eligible to drive
    else if(age >= 18){
        var message  = `age <strong>${age}</strong> is eligible to drive`   // message to be displayed to user
        displayResult(result,error,info,message=message,status="success")
    }
    //  if age is less than 18 then the user is too young to drive
    else{
        var message  = `age <strong>${age}</strong> is too young to drive`   // message to be displayed to user
        displayResult(result,error,info,message=message,status="info")
    }
